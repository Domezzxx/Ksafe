...
export default function App() {
  ...
  // ✅ 1. เพิ่ม State สำหรับเก็บค่าการกรองสี (Filter)
  ...
  const navigateTo = {
    ...
    // ✅ 2. ฟังก์ชันเปลี่ยนหน้า Sorting และจำค่าสีที่กดมา
  ...
  // 4. Admin Screens (✅ ส่วนที่สำคัญที่สุด)
  ...
  if (currentScreen === 'AdminHome') {
    return (
      <AdminHomeScreen 
        ...
        onGoToSorting={navigateTo.incidentSorting} // ✅ ส่งฟังก์ชันนี้ไปให้หน้าแอดมิน
      />
    );
  }
  ...
  if (currentScreen === 'SOS') return <SosScreen onCancel={navigateTo.home} onGoHome={navigateTo.home} onGoSearch={navigateTo.search} onGoProfile={navigateTo.profile} />;
  if (currentScreen === 'Profile') return <ProfileScreen currentUserPhone={userPhone} onGoHome={userRole === 'admin' ? navigateTo.admin : navigateTo.home} onGoSOS={userRole === 'admin' ? navigateTo.manageContact : navigateTo.sos} onGoSearch={userRole === 'admin' ? navigateTo.manageFacilities : navigateTo.search} onGoProfile={navigateTo.profile} onLogout={() => { setUserRole(''); setUserPhone(''); setCurrentScreen('Login'); }} />;
