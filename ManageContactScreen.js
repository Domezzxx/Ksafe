...
import { LayoutDashboard, PhoneCall, MapPinned, UserCog, Edit2, Plus, Search } from 'lucide-react-native';

const { width } = Dimensions.get('window');
const categories = ['โรงพยาบาล', 'สถานีตำรวจ', 'กู้ภัย', 'สถานีดับเพลิง','อื่นๆ']; // ตัด 'ทั้งหมด' ออกสำหรับหน้าเพิ่มข้อมูล
...
export default function ManageContactScreen({
  ...
  const [category, setCategory] = useState('กู้ภัย'); // กำหนดค่าเริ่มต้น
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (image && image.startsWith('file')) {
        ...
        const storageRef = ref(storage, `emergency/${Date.now()}.jpg`);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity style={styles.footerButton} onPress={onGoSOS}>
            <Image source={require('./assets/emergency (1).png')} style={[styles.footerIcon, { tintColor: '#F87C47' }]} />
          </TouchableOpacity>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={styles.formArea}>
            <View style={styles.inputGroup}>
              ...
              <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="เช่น โรงพยาบาลพระปกเกล้า" />
            </View>
            ...
            {/* ส่วนเลือกหมวดหมู่แบบปุ่ม (Radio) */}
            <View style={styles.inputGroup}>
              ...
              {image ? (
                ...
                <View style={{alignItems: 'center'}}>
                   <View style={styles.plusCircle}>
                    <Plus size={28} color="#F48E54" />
                   </View>
                   <Text style={{color: '#999', marginTop: 12, fontWeight: '500'}}>เลือกรูปภาพหน่วยงาน</Text>
                </View>
              )}
            </TouchableOpacity>
            ...
            <TouchableOpacity onPress={resetForm} style={styles.cancelBtn}>
               <Text style={{ color: '#999', fontWeight: '600' }}>ยกเลิกและย้อนกลับ</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  fab: { 
    position: 'absolute', right: 25, bottom: 120, backgroundColor: '#F48E54', 
    width: 64, height: 64, borderRadius: 32, justifyContent: 'center', alignItems: 'center', elevation: 8 
  },
  ...
  imagePicker: { 
    height: 200, borderRadius: 24, borderStyle: 'dashed', borderWidth: 2, borderColor: '#F48E54',
    backgroundColor: '#FFF7ED', justifyContent: 'center', alignItems: 'center', marginBottom: 30, overflow: 'hidden'
  },
  ...
  footer: {
    position: 'absolute', bottom: 0, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center',
    width: '100%', height: 80, backgroundColor: '#FFF', borderTopWidth: 1, borderTopColor: '#F0F0F0', paddingBottom: 15
  },
