    ...
    (async () => {
      ...
      locationSubscription = await Location.watchPositionAsync(
        ...
        (newLocation) => {
          ...
          // เลื่อนกล้องแผนที่ให้ตามหมุดอัตโนมัติ
          if (mapRef.current) {
            mapRef.current.animateToRegion({
              ...currentPos,
              latitudeDelta: 0.01, // ปรับให้ซูมใกล้ๆ จะได้เห็นหมุดชัดๆ
              longitudeDelta: 0.01,
            }, 1000); 
          }
        }
...
const styles = StyleSheet.create({
  ...
  // 📍 สไตล์จุดสีส้มที่ตั้งต้น
  orangeDotMarker: { 
    width: 22, 
    height: 22, 
    borderRadius: 11, 
    backgroundColor: '#F7934C', // สีส้มเดียวกับธีม
    borderWidth: 3, 
    borderColor: '#FFFFFF', // ขอบขาวให้ดูเด่น
    elevation: 4, // เพิ่มเงาใน Android
    shadowColor: '#000', // เพิ่มเงาใน iOS
